...
const courseData = [
  ...
  { "course_code": "UIL-GGEY 303", "course_title": "Geology of Osi Sheet", "id": "a9c9639c-6d28-466c-98cd-8d8f787d2224" },
...
export default function UploadMaterialPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(`${API_URL}/materials/upload`, data, {
                headers: { 'Authorization': `Bearer ${token}` },
            });

            setMessage(`Success! Material uploaded. ID: ${response.data.id}`);
            setIsSuccess(true);
            setFormData({ title: '', material_type: 'Past Question', course_id: formData.course_id, file: null });

        } catch (error) {
            console.error("Upload Error:", error.response || error);
            const detail = error.response?.data?.detail || "Upload failed. Check console for API error.";
    ...
    if (!user) { // Don't render until we know who the user is
         return (
            <div style={{
                display: 'flex', 
                minHeight: '100vh', 
        ...
        <DashboardLayout user={user}>
            <div className="space-y-6">
                ...
                <motion.div
                    ...
                    className="metric-card" // <-- USE THE NEW LIGHT CARD
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="form-input-group pt-4">
                            ...
                            <input
                                ...
                                className="form-input" 
                                style={{ padding: '0.5rem', height: 'auto', backgroundColor: 'var(--color-bg-light)', color: 'var(--color-text-dark)' }}
                            />
                        </div>
